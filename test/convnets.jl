...
@testset "ResNet" begin
    ...
    @testset "resnet" begin @testset for block_fn in [
    ...
    @testset "WideResNet" begin @testset "WideResNet($sz)" for sz in [50, 101]
        m = WideResNet(sz)
        @test size(m(x_224)) == (1000, 1)
        @test gradtest(m, x_224)
        _gc()
    ...
    @testset for cardinality in [32, 64]
        @testset for base_width in [4, 8]
            ...
            if (ResNeXt, depth, cardinality, base_width) in PRETRAINED_MODELS
                ...
                @test_throws ArgumentError ResNeXt(depth; cardinality, base_width,
                                                   pretrain = true)
            end
            @test gradtest(m, x_224)
            _gc()
...
@testset "SEResNet" begin @testset for depth in [18, 34, 50, 101, 152]
    m = SEResNet(depth)
    @test size(m(x_224)) == (1000, 1)
...
@testset "SEResNeXt" begin @testset for depth in [50, 101, 152]
    @testset for cardinality in [32, 64]
        @testset for base_width in [4, 8]
...
@testset "Res2NeXt" begin @testset for depth in [50, 101]
    m = Res2NeXt(depth)
    @test size(m(x_224)) == (1000, 1)
...
@testset "EfficientNet" begin @testset "EfficientNet($config)" for config in [
    :b0,
    :b1,
    :b2,
    :b3,
...
@testset "EfficientNetv2" begin @testset for config in [:small, :medium, :large] # :xlarge]
...
@testset "GoogLeNet" begin @testset for bn in [true, false]
...
@testset "MobileNets (width = $width_mult)" for width_mult in [0.5, 0.75, 1, 1.3]
    ...
    _gc()
    @testset "MobileNetv3" verbose=true begin @testset for config in [:small, :large]
    ...
    @testset "MNASNet" verbose=true begin @testset for config in [:A1, :B1]
...
@testset "ConvNeXt" verbose=true begin @testset for config in [
    :small,
    :base,
    :large,
    :tiny,
...
@testset "ConvMixer" verbose=true begin @testset for config in [:small, :base, :large]
    m = ConvMixer(config)
    @test size(m(x_224)) == (1000, 1)
    @test gradtest(m, x_224)
    _gc()
