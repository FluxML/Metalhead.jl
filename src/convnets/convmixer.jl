"""
    convmixer(planes, depth; inchannels = 3, kernel_size = (9, 9), patch_size = 7,
...
"""
function convmixer(planes, depth; inchannels = 3, kernel_size = (9, 9), patch_size = 7,
                   activation = gelu, nclasses = 1000)
  stem = conv_bn((patch_size, patch_size), inchannels, planes, activation; preact = true, stride = patch_size)
  ...
  pool = Chain(AdaptiveMeanPool((1, 1)), MLUtils.flatten)
  head = Dense(planes, nclasses)
  return Chain(Chain(stem..., blocks..., pool),  head)
end
