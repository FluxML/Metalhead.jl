...
function SwinTransformer(img_size::Tuple,patch_size::Tuple,window_size::Tuple,n_classes::Int,embed_dim::Int,depths=[2,2,6,2],num_heads=[3,6,12,24],mlp_ratio=4.,qkv_bias=true,qk_scale=nothing,drop_rate=0.,attn_drop_rate=0.,drop_path_rate=0.1,norm_layer=LayerNorm,ape=false, patch_norm=true)
    ape=ape;
    patchEmbed=Chain(Layers.PatchEmbedding(patch_size),Dropout(drop_rate));
    ...
    sdpr=[x for x in 0:drop_path_rate:sum(depths)];
    seq_layers=[]
    if qk_scale===nothing
        for i in 1:length(depths)
            dim = Int(embed_dim * 2 ^ (i-1));
            downsample=Layers.PatchMerging(img_size,dim,norm_layer);
            append!(seq_layers,BasicLayer(dim,img_size,depths[i],num_heads[i],window_size,mlp_ratio,
            qkv_bias,(dim/num_heads[i])^-0.5,drop_rate,attn_drop_rate,sdpr[sum(depths[begin:i]):sum(depths[begin:i+1])],downsample))
        end
    else
        for i in 1:length(depths)
            dim = Int(embed_dim * 2 ^ (i-1));
            downsample=Layers.PatchMerging(img_size,dim,norm_layer);
            append!(seq_layers,BasicLayer(dim,img_size,depths[i],num_heads[i],window_size,mlp_ratio,
            qkv_bias,qk_scale,drop_rate,attn_drop_rate,sdpr[sum(depths[begin:i]):sum(depths[begin:i+1])],downsample))
        end
    end
